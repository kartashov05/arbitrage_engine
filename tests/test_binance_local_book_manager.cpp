...
TEST(BinanceLocalBookManagerTest, BuffersUpdatesBeforeSnapshot) {
    arb::BinanceLocalBookManager manager{"BTCUSDT"};
    ...
    EXPECT_EQ(result, arb::LocalBookUpdateResult::Buffered);
    EXPECT_EQ(manager.buffered_updates(), 1);
    EXPECT_EQ(manager.status(), arb::LocalBookStatus::WaitingForSnapshot);
    EXPECT_FALSE(manager.ready());
}

TEST(BinanceLocalBookManagerTest, AppliesSnapshotAndBufferedUpdates) {
    arb::BinanceLocalBookManager manager{"BTCUSDT"};
    ...
    EXPECT_EQ(result, arb::LocalBookUpdateResult::SnapshotApplied);
    EXPECT_TRUE(manager.ready());
    EXPECT_EQ(manager.buffered_updates(), 0);

    const auto& book = manager.book();

    EXPECT_EQ(book.last_update_id(), 102);
...
TEST(BinanceLocalBookManagerTest, RejectsSnapshotWhenBufferedUpdatesDoNotBridgeSnapshot) {
    arb::BinanceLocalBookManager manager{"BTCUSDT"};
    ...
    EXPECT_EQ(result, arb::LocalBookUpdateResult::SnapshotRejected);
    EXPECT_TRUE(manager.need_resync());
    EXPECT_EQ(manager.buffered_updates(), 0);
}

TEST(BinanceLocalBookManagerTest, DetectsGapAfterBookIsReady) {
    arb::BinanceLocalBookManager manager{"BTCUSDT"};

    const auto snapshot_result = manager.apply_snapshot(make_snapshot(100));

    EXPECT_EQ(snapshot_result, arb::LocalBookUpdateResult::SnapshotApplied);
    EXPECT_TRUE(manager.ready());
    ...
    EXPECT_EQ(update_result, arb::LocalBookUpdateResult::GapDetected);
    EXPECT_TRUE(manager.need_resync());
}

TEST(BinanceLocalBookManagerTest, IgnoresUpdatesForAnotherSymbol) {
    arb::BinanceLocalBookManager manager{"BTCUSDT"};
    ...
    EXPECT_EQ(result, arb::LocalBookUpdateResult::NotForSymbol);
    EXPECT_EQ(manager.buffered_updates(), 0);
}

TEST(BinanceLocalBookManagerTest, ResetReturnsManagerToWaitingState) {
    arb::BinanceLocalBookManager manager{"BTCUSDT"};
