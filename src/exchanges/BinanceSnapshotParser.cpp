#include "BinanceSnapshotParser.hpp"
...
BookSnapshot BinanceSnapshotParser::parse(
    ...
    if (!root.contains("lastUpdateId")) {
        throw std::runtime_error("Binance snapshot missing lastUpdateId");
    }

    if (!root.contains("bids") || !root.contains("asks")) {
        throw std::runtime_error("Binance snapshot missing bids/asks");
    }
