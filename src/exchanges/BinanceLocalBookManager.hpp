...
enum class LocalBookStatus {
    WaitingForSnapshot,
    Ready,
    NeedResync
};

enum class LocalBookUpdateResult {
    Buffered,
    SnapshotApplied,
    SnapshotRejected,
    Applied,
...
class BinanceLocalBookManager {
public:
    explicit BinanceLocalBookManager(
        std::string symbol,
        std::size_t max_buffered_updates = 10000
    );

    LocalBookUpdateResult on_update(const BookUpdate& update);
    LocalBookUpdateResult apply_snapshot(const BookSnapshot& snapshot);

    [[nodiscard]] const OrderBook& book() const;

    [[nodiscard]] const std::string& symbol() const;
    [[nodiscard]] LocalBookStatus status() const;
    [[nodiscard]] bool ready() const;
    [[nodiscard]] bool need_resync() const;

    [[nodiscard]] std::size_t buffered_updates() const;
    [[nodiscard]] LocalBookStats stats() const;
...
private:
    ...
    [[nodiscard]] LocalBookUpdateResult apply_update_to_ready_book(
...
private:
    ...
    LocalBookStatus status_{LocalBookStatus::WaitingForSnapshot};
    LocalBookStats stats_;
};
